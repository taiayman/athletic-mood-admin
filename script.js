...
// Global functions for auth state management
...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    showDashboard();
    // Load admin data after DOM is ready
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => loadAdminData(user));
    } else {
      loadAdminData(user);
    }
  } else {
...
document.addEventListener("DOMContentLoaded", () => {
  // Check if db and auth are initialized
  if (typeof db === "undefined" || typeof auth === "undefined") {
    console.error(
      `Firebase services (db/auth) are not initialized. ` +
        `Check HTML script order and config.`
    );
    alert(
  ...
  const reportedAdDetailsModal = document.getElementById(
    "reported-ad-details-modal"
  );
  const reportedAdInfoDiv = document.getElementById("reported-ad-info");
  const reportedAdReasonsListDiv = document.getElementById(
    "reported-ad-reasons-list"
  );
  const deleteAdBtnInModal = document.getElementById("delete-ad-btn");
  // Add other element selectors
  // from the new modal as needed, e.g., spans for details
  ...
  const detailAdReportCountSpan = document.getElementById(
    "detail-ad-report-count"
  );
  ...
  const fetchData = async (collectionPath, options = {}) => {
    ...
    console.log(
      `Fetched ${snapshot.docs.length} documents from ${collectionPath}`
    );

    // Also get total count for pagination (might need separate query
    // or Cloud Function for large datasets)
    // For simplicity here, we fetch total based on filters only
    // (less accurate for large sets)
    ...
    // We'll estimate total based on whether we got a full page back,
    // or use a fixed large number.
    ...
    const approxTotal =
      snapshot.docs.length < limitCount
        ? (currentPage - 1) * limitCount + snapshot.docs.length
        : currentPage * limitCount + 1; // Rough estimate
  ...
  const fetchUsers = async (options = {}) => {
    ...
    const userTypeFilterValue = filters.find(
      (f) => f.field === "userType"
    )?.value;
    const userSearchQuery = filters
      .find((f) => f.field === "search")
      ?.value?.toLowerCase();
    ...
    if (fetchOnlyPending) {
      ...
      let allPendingUsers = [];
      // This will be tricky for true pagination across multiple queries
      ...
      for (const col of pendingCollections) {
        let pendingQuery = db
          ...
          // Assuming pending users must be verified
          .where("isVerified", "==", true);
        // .orderBy('updatedAt', 'desc');
        // // Or 'createdAt', consistent across collections
        // <-- REMOVED THIS LINE

        // Note: Firestore pagination (startAfterDoc) is complex
        // with multiple queries.
        // For simplicity in this tab, we might fetch all
        // or implement client-side pagination for the combined list.
        // Or, if the number of pending users is generally small,
        // fetch all.
        // For now, we will fetch up to a larger limit
        // per collection and combine.
        // A more robust solution might need a dedicated
        // Cloud Function or separate fetches per type
        // if counts are large.

        // Let's fetch a moderate number from each and combine.
        // True server-side pagination is hard here.
        // We'll ignore startAfterDoc for
        // combined pending fetches for now, and rely on limitCount
        // or a fixed larger fetch.
        // This might mean pagination for the combined pending list
        // is simpler (e.g. client-side if total is manageable)
        // or just shows the first N results.

        // Fetch more initially, up to 3x itemsPerPage
        const snapshot = await pendingQuery.limit(limitCount * 3).get();
        snapshot.docs.forEach((doc) => {
          // 'professionals' -> 'professional'
          let userTypeFromCollection = col.slice(0, -1);
          allPendingUsers.push({
            id: doc.id,
            ...doc.data(),
            userType: doc.data().userType || userTypeFromCollection,
          });
        });
        // totalPendingCount += snapshot.size;
        // This would be per-collection count if not limited
      }

      // Sort all pending users
      // (e.g., by email or a timestamp if available and consistent)
      ...
      // Total count of the combined list
      totalPendingCount = allPendingUsers.length;

      // For pagination of the combined list, if we fetched all:
      // const startIndex = (currentPage - 1) * limitCount;
      // const paginatedUsers =
      // 	allPendingUsers.slice(startIndex, startIndex + limitCount);
      // lastFetchedDoc could be null or an indicator
      // if more pages exist for client-side pagination.

      // For now, returning up to limitCount of the sorted combined list
      const paginatedUsers = allPendingUsers.slice(0, limitCount);
      // This simplified pagination for pending means 'lastDoc'
      // isn't directly from Firestore for the *next* combined page.

      // Return the first page of combined, sorted pending users
      return {
        users: paginatedUsers,
        // Indicates if there are more users than shown
        lastDoc: allPendingUsers.length > limitCount ? true : null,
      ...
      // This part remains for the main User Management tab
      // when filtering by Professionals
      if (filters.length > 0) {
        filters.forEach((filter) => {
          if (filter.field !== "userType" && filter.field !== "search") {
            professionalQuery = professionalQuery.where(
              filter.field,
              filter.operator,
              filter.value
            );
      ...
      if (userSearchQuery) {
        professionals = professionals.filter(
          (prof) =>
            (prof.businessName?.toLowerCase() || "").includes(
              userSearchQuery
            ) ||
      ...
      if (filters.length > 0) {
        filters.forEach((filter) => {
          if (filter.field !== "userType" && filter.field !== "search") {
            countQueryTotal = countQueryTotal.where(
              filter.field,
              filter.operator,
              filter.value
            );
      ...
      // Existing logic for fetching combined users if filter
      // is not 'professional' or is 'all'
      ...
      for (const col of collections) {
        if (
          ...
          // col is plural, type is singular
          userTypeFilterValue === col.slice(0, -1)
        ) {
          try {
            // Pass down sortBy, sortOrder, limitCount,
            // startAfterDoc for individual collection fetches
            const fetchOptionsForCollection = {
              // Remove global filters
              filters: filters.filter(
                (f) => f.field !== "userType" && f.field !== "search"
              ),
              sortBy: options.sortBy,
              sortOrder: options.sortOrder,
              limitCount: limitCount,
              // This is tricky with multiple collections,
              // needs refinement for true pagination
              startAfterDoc: startAfterDoc,
            };
            // If a specific type is filtered (not 'all'),
            // only query that collection
            if (
              userTypeFilterValue &&
              userTypeFilterValue !== "all" &&
              col !== `${userTypeFilterValue}s`
            ) {
              //skip if not the selected type
            } else {
              const { docs, lastDoc, totalCount } = await fetchData(
                col,
                fetchOptionsForCollection
              );
              allUsers.push(
                ...docs.map((d) => ({
                  ...d,
                  userType: d.userType || col.slice(0, -1),
                }))
              ); // ensure userType
      ...
      return {
        users: uniqueUsers,
        lastDoc: combinedLastDoc,
        totalCount: uniqueUsers.length,
      }; // totalCount is approximate here
    }
  };
  ...
  const firebaseUpdateUser = async (userId, userData) => {
    ...
    try {
      // Use update instead of set merge to avoid creating doc
  ...
  const firebaseDeleteUser = async (userId) => {
    ...
    try {
      ...
      // Consider deleting associated Firebase Auth user if applicable
      // Consider deleting related data (activities, teams etc.)
      // DANGEROUS, use Cloud Functions ideally
      return deleted;
    } catch (error) {
  ...
  const firebaseAddUser = async (userData) => {
    ...
    switch (userData.type) {
      // Assuming userData has a 'type' field from modal
      case "professional":
        collectionPath = "professionals";
        break;
      case "club":
  ...
  const firebaseDeleteActivity = async (activityId) => {
    ...
    const activityRef = db.collection("activities").doc(activityId);
    try {
      // Optional: Delete messages subcollection first
      // (use Cloud Function for safety)
  ...
  const firebaseUpdateFacility = async (facilityId, data) => {
    ...
    try {
      // Remove arenaId if it exists in data,
      // as it shouldn't change normally
      delete data.arenaId;
  ...
  const renderUserTable = (users) => {
    ...
    users.forEach((user) => {
      ...
      if (
        currentUserTypeFilter === "professional" &&
        ...
        row.innerHTML = `
                    ...
                    <td>
						<span class="status-badge type-professional">
							Professional
						</span>
					</td>
                    <td>
						<span class="status-badge status-${isVerified ? "active" : "inactive"}">${
          isVerified ? "Yes" : "No"
        }
						</span>
						</td>
                    <td>
						<span class="status-badge status-${isApproved ? "active" : "pending"}">${
          isApproved ? "Yes" : "No"
        }
						</span>
					</td>
                    ...
                    <td class="actions">
                        <button class="btn btn-sm btn-approve ${
                          isApproved ? "btn-disabled" : "btn-success"
                        }" 
                                data-id="${user.id}" 
        ...
        const userType =
          user.userType ||
          (user.hasOwnProperty("clubName")
            ? "club"
            : user.hasOwnProperty("arenaName")
            ? "arena"
            : "individual");
        row.innerHTML = `
                    <td>${
                    ...
                    <td>
						<span class="status-badge type-${userType}">
							${userType}
						</span>
					</td>
  ...
  const renderPendingApprovalTable = (users) => {
    const tableBody = document
      .getElementById("pending-approval-table")
      ?.querySelector("tbody");
    if (!tableBody) {
      console.error(
        `element with ID 'pending-approval-table and a tbody not found.`
      );
      return;
    }
    tableBody.innerHTML = "";
    ...
    users.forEach((user) => {
      // This function now expects users from professionals,
      // clubs, or arenas
      ...
      const displayName =
        ...
        "N/A";
      const isVerified = user.isVerified === true;
      // Should be false for this table
      const isApprovedForDisplay = user.isApproved === true;
      ...
      row.innerHTML = `
                ...
                <td>
					<span class="status-badge type-${user.userType || "unknown"}">
						${userTypeDisplay}
					</span>
				</td>
                <td>
            ...
            isApprovedForDisplay ? "active" : "pending"
          }">
							${isApprovedForDisplay ? "Yes" : "Pending"}
					</span>
				</td>
                <td>${formatDate(user.createdAt || user.updatedAt)}</td>
                <td class="actions">
  ...
  const renderActivityTable = (activities) => {
    ...
    activities.forEach((activity) => {
      const isBoosted =
        activity.isBoosted &&
        (!activity.boostExpiryDate ||
          activity.boostExpiryDate.toDate() > new Date());
      ...
      row.innerHTML = `
                ...
                <td>
					<span class="status-badge ${isBoosted ? "boosted" : "default"}">
						${isBoosted ? "Boosted" : "No"}
					</span>
				</td>
                <td class="actions">
                    <button
  ...
  const renderEventTable = (events) => {
    ...
    events.forEach((event) => {
      ...
      const eventTypeLabel = event.isTournament
        ? "Tournament"
        : event.isForCouples
        ? "Couples"
        : event.isForSingles
      ...
      row.innerHTML = `
                ...
                <td>
					<span class="status-badge ${isPaid ? "paid" : "free"}">
						${isPaid ? `Paid (${price.toFixed(2)}€)` : "Free"}
					</span>
				</td>
                <td class="actions">
                	<button
  ...
  const renderTeamTable = (teams) => {
    ...
    document.getElementById("team-details-container").style.display =
      teams.length > 0 ? "block" : "none";
  };

  const renderArenaTable = (arenas) => {
    const tableBody = document
      .getElementById("arena-table")
      ?.querySelector("tbody");
    if (!tableBody) return;
    tableBody.innerHTML = "";
    arenas.forEach((arena) => {
      ...
      row.innerHTML = `
                ...
                <td>${facilityCount}</td>
                 <td class="actions">
                    <button
						class="view-facilities-btn"
						data-id="${arena.id}"
						data-name="${arena.arenaName}"
  ...
  const renderFacilityTable = (facilities) => {
    ...
    facilities.forEach((facility) => {
      const row = tableBody.insertRow();
      const statusClass =
        facility.status?.toLowerCase().replace(/[^a-z0-9]/g, "") || "default";
      row.innerHTML = `
                <td>${facility.name}</td>
                <td>${facility.type}</td>
                 <td class="actions">
    ...
    document.getElementById("facility-details-container").style.display =
      facilities.length > 0 ? "block" : "none";
  };

  // --- New Function to Render Professional Balances Table ---
  // Changed professionals to entities
  const renderProfessionalBalanceTable = (entities) => {
    const tableBody = document
      .getElementById("professional-balance-table")
      ?.querySelector("tbody");
    if (!tableBody) return;
    tableBody.innerHTML = "";
    entities.forEach((entity) => {
      // Changed pro to entity
      const row = tableBody.insertRow();
  ...
  // Potentially adapt for arenas
  // Added entityType
  const displayPayoutInfo = (entityName, payoutInfo, entityType) => {
    ...
    // Consider renaming ID if generic
    const nameSpan = document.getElementById(
      "selected-professional-payout-name"
    );
    ...
    if (!payoutInfo || Object.keys(payoutInfo).length === 0) {
      contentDiv.innerHTML = `<p>
					<em>No bank details found for this ${entityType}.</em>
				</p>`;
    } else {
      // formatDataForDisplay should be generic enough
  ...
  const renderBookingTable = (bookings) => {
    ...
    bookings.forEach((booking) => {
      ...
      const facilityName =
        ...
        `Facility (${booking.facilityId.substring(0, 5)}...)`;
      const userName =
        booking.userName || `User (${booking.userId.substring(0, 5)}...)`;
      row.innerHTML = `
                <td>${facilityName}</td>
                <td>${userName}</td>
  ...
  const renderConversationTable = (conversations) => {
    const tableBody = document
      .getElementById("conversation-table")
      ?.querySelector("tbody");
    if (!tableBody) return;
    tableBody.innerHTML = "";
      ...
      row.innerHTML = `
                ...
                <td class="actions">
                	<button
						class="view-btn"
						data-id="${conv.id}"
						title="View Messages (Not Implemented)">
						<i class="fas fa-eye"></i>
					</button>
                    <button
						class="delete-btn"
  ...
  const fetchReportedActivities = async (options = {}) => {
    ...
    // (e.g., search, though search might be client-side
    // for simplicity here)
    filters.forEach((filter) => {
      if (filter.field && filter.operator && filter.value !== undefined) {
        query = query.where(filter.field, filter.operator, filter.value);
    ...
    if (sortBy) {
      query = query.orderBy(sortBy, sortOrder);
      // If sorting by a field other than reportCount,
      // and reportCount is used in the primary where clause,
      // Firestore might require a composite index.
      // For reportCount > 0, this orderBy should be fine.
      // If also ordering by timestamp for example (e.g. lastReportedAt),
      // ensure `reportCount` is the first orderBy
      // if not part of an equality filter.
      // Or, if `lastReportedAt` is always updated when `reportCount` is,
      // sorting by `lastReportedAt` might be more relevant.
      // Let's assume for now `reportCount` is the primary sort,
      // or `lastReportedAt` if specified.
      if (sortBy !== "reportCount") {
        // Example: if primary sort is something else
        // like 'lastReportedAt'
    ...
    try {
      ...
      // For total count, query without limit and pagination
      // for accuracy based on the primary filter
      ...
      // .size provides the count of documents in the QuerySnapshot
      const totalCount = totalSnapshot.size;

      console.log(`Fetched ${activities.length} 
				reported activities, total: ${totalCount}`);
  ...
  const renderReportedAdsTable = (activities) => {
    ...
    if (!activities || activities.length === 0) {
      tableBody.innerHTML = `
				<tr>
					<td colspan="5" class="placeholder-text">
						No reported ads found.
					</td>
				</tr>`;
      return;
    }
    ...
    activities.forEach((ad) => {
      ...
      row.innerHTML = `
                ...
                <td>${
                  ad.lastReportedAt ? formatDateTime(ad.lastReportedAt) : "N/A"
                }
				</td>
                <td>${ad.creatorId || "Unknown"}</td>
                <td class="actions">
                    <button
						class="btn btn-small view-reported-ad-details-btn"
						data-id="${ad.id}"
						title="View Details">
						<i class="fas fa-eye"></i>
						View
                    ...
                    <button
						class="btn btn-small btn-danger delete-reported-ad-btn"
						data-id="${ad.id}"
						title="Delete Ad">
						<i class="fas fa-trash"></i>
						Delete
  ...
  const fetchActivityReports = async (activityId) => {
    try {
      ...
      console.error(
        `Error fetching reports for activity ${activityId}:`,
        error
      );
  ...
  const formatActivityDetailsForModal = (activity, reports) => {
    ...
    // Use helper functions for dates/times
    // if they exist (like formatDate, formatDateTime)
    const formattedDateTime = activity.dateTime
      ? formatDateTime(activity.dateTime)
      : "N/A";
    const creatorInfo = activity.creatorName
      ? `${activity.creatorName} (ID: ${activity.creatorId})`
      : activity.creatorId || "Unknown";
    // Adjust based on how level is stored
    const levelDisplayName =
      activity.level?.displayName || activity.level || "N/A";
    // Adjust based on how ageGroup is stored
    const ageGroupDisplayName =
      activity.ageGroup?.displayName || activity.ageGroup || "N/A";
    const participantsText = `${activity.participants?.length ?? 0}/
			${activity.maxParticipants ?? "N/A"}`;
    const priceText = activity.isPaid
    ...
    if (reports && reports.length > 0) {
      ...
      reportsHtml += `
				<p>
					<em>No reports found for this activity.</em>
				</p>`;
    }

    // Structure HTML similar to Flutter page sections
    // Assuming .detail-item and .full-width classes exist
    // and provide basic styling
    const htmlContent = `
            <div style="
				display: grid; 
				...
				gap: 15px;">
                	...
                	<div class="detail-item">
						<strong>Level:</strong> ${levelDisplayName}
					</div>
                	<div class="detail-item">
						<strong>Age Group:</strong> ${ageGroupDisplayName}</div>
                	<div class="detail-item">
						<strong>Participants:</strong> ${participantsText}</div>
                	<div class="detail-item">
						<strong>Price:</strong> ${priceText}
					</div>
                	<div class="detail-item">
						<strong>Creator:</strong> ${creatorInfo}</div>
                	<div class="detail-item">
						<strong>Creator Type:</strong> 
							${activity.creatorType || "N/A"}
					</div>
                	<div class="detail-item">
                  ...
                  activity.equipment && activity.equipment.length > 0
                    ? `
						<div class="detail-item full-width">
							...
							<br>
							<ul>
								<li>${activity.equipment.join("</li><li>")}</li>
							</ul>
						</div>`
                    : ""
                }
  ...
  const showFullActivityDetailsModal = async (activityId) => {
    ...
    try {
      ...
      // Fetch reports data
      // Use existing helper
      const reportsData = await fetchActivityReports(activityId);

      // Format the content
      const modalTitle = `Activity Details: ${activityData.name || activityId}`;
      const modalContentHtml = formatActivityDetailsForModal(
        activityData,
        reportsData
      );
      ...
      console.error(
        "Error fetching or displaying full activity details:",
        error
      );
      showSnackbar("Failed to load activity details.", "error");
      // Optionally show the error in the modal itself
      showDetailsModal(
        `Error Loading Activity ${activityId}`,
        `<p style="color: var(--error-red);">
					Could not load details. Please check the console.
				</p>`
      );
    }
  };
  ...
  const loadSectionData = async (sectionId) => {
    // New Log
    console.log(`loadSectionData: Called for section: ${sectionId}`);
    // Display loading state in the relevant table
    const tableBody = document.querySelector(`#${sectionId} table tbody`);
    // Get the container for boosted announcements
    const boostedAnnouncementsListContainer = document.getElementById(
      "boosted-announcements-list"
    );

    if (sectionId === "boosted-announcements-management") {
      if (boostedAnnouncementsListContainer) {
        console.log(`loadSectionData: Setting loading message for
					boosted-announcements-list.`);
        boostedAnnouncementsListContainer.innerHTML =
          '<p class="loading-text">Loading announcements...</p>';
      } else {
        console.warn(`loadSectionData: boosted-announcements-list
					container not found!`);
      }
    } else if (tableBody) {
      tableBody.innerHTML = `
    ...
    try {
      ...
      const bookingStatusFilter = document.getElementById(
        "booking-status-filter"
      )?.value;
      const chatSearch = document.getElementById("chat-search")?.value;
      const proBalanceSearch =
        document.getElementById("pro-balance-search")?.value;
      // New Log
      console.log(`loadSectionData: Preparing to switch for sectionId:
				${sectionId}`);
      switch (sectionId) {
        case "dashboard-overview":
          // Fetch counts for overview cards
          // (Simplified - real app needs aggregation)
          // Estimate
          ...
          document.getElementById("total-users").textContent =
            usersSnap.size + prosSnap.size + clubsSnap.size + arenasSnap.size;
        ...
        case "user-management":
          ...
          if (userSearch) {
            // Add a 'search' field to options
            // for client-side filtering,
            // as Firestore doesn't support partial text search
            // well across multiple fields.
          ...
          // Track collection for pagination
        ...
        case "club-management":
          ...
          // We need team count -
          // this requires extra queries or denormalization
        ...
        case "arena-management":
          ...
          const {
            docs: arenas,
            lastDoc: arenaLastDoc,
            totalCount: arenaTotal,
          } = await fetchData("arenas", {
            filters,
            sortBy,
            startAfterDoc: lastVisibleDoc,
          });
          // We need facility count -
          // this requires extra queries or denormalization
          // Simulating adding facilityCount for now
          const arenasWithCount = await Promise.all(
            arenas.map(async (a) => {
        ...
        // Needs backend/client-side logic
        case "booking-management":
          ...
          } = await fetchData("facility_bookings", {
            filters,
            sortBy,
            sortOrder,
            startAfterDoc: lastVisibleDoc,
          ...
          const populatedBookings = await Promise.all(
            bookings.map(async (b) => {
              const facDoc = await db
                .collection("facilities")
                .doc(b.facilityId)
                .get();
              // Use helper
              const user = await getUserData(b.userId);
              return {
                ...b,
                facilityName: facDoc.exists
                  ? facDoc.data()?.name
                  : "Unknown Facility",
        ...
        case "chat-management":
          // Client-side search
          if (chatSearch) filters.push({ field: "search", value: chatSearch });
          ...
          } = await fetchData("conversations", {
            filters,
            sortBy,
            sortOrder,
            startAfterDoc: lastVisibleDoc,
          });
          // Populate necessary display info (participants etc.)
          // - could be complex
          const populatedConvs = await Promise.all(
            conversations.map(async (c) => {
              const type = c.isEventChat
                ? "Event"
                : c.activityId
                ? "Activity"
          ...
          if (chatSearch) {
            ...
            // if searching by participant, then filter
            console.warn(`Participant search in chat not
							fully implemented client-side`);
            finalConvs = populatedConvs.filter((c) =>
        ...
        case "professional-balances":
          // if (proBalanceSearch) {
          // Client-side search was commented out, keeping it so
          //     filters.push({
          // 	field: 'search',
          // 	value: proBalanceSearch });
          // }
          // sortBy = 'balance'; // Default sort by balance descending
          // sortOrder = 'desc'; // Default sort by balance descending
          // Commenting out default sort to apply after merging
          // Meta-collection name for pagination/UI
          currentCollection = "financials";
          ...
          try {
            ...
            // --- Ensure only professionals
            // with balance > 0 are fetched ---
            proFilters.unshift({
              field: "balance",
              operator: ">",
              value: 0,
            ...
            professionals.forEach((p) =>
              allEntitiesWithBalance.push({
                ...p,
                userType: "professional",
                displayName: p.businessName || p.fullName || p.email || p.id,
              })
            );
            lastProDoc = proLD;
          ...
          try {
            ...
            // --- Ensure only arenas with balance > 0
            // are fetched ---
            arenaFilters.unshift({
              field: "balance",
              operator: ">",
              value: 0,
            });
            ...
            arenas.forEach((a) =>
              allEntitiesWithBalance.push({
                ...a,
                userType: "arena",
                displayName: a.arenaName || a.email || a.id,
              })
            );
          ...
          // Client-side search (if proBalanceSearch is active
          // and filters in fetchData are not sufficient)
          if (proBalanceSearch) {
            const query = proBalanceSearch.toLowerCase();
            allEntitiesWithBalance = allEntitiesWithBalance.filter(
              (e) =>
          ...
          const paginatedEntities = allEntitiesWithBalance.slice(
            startIndex,
            startIndex + itemsPerPage
          );

          renderProfessionalBalanceTable(paginatedEntities);
          // lastVisibleDoc needs to be handled
          // carefully for combined sources.
          // For simple client-side pagination of the combined list:
          lastVisibleDoc =
            startIndex + itemsPerPage < allEntitiesWithBalance.length
              ? true
              : null;
          totalItems = allEntitiesWithBalance.length;

          document.getElementById("payout-info-container").style.display =
            "none";
          break;

        case "boosted-announcements-management": // ADDED THIS CASE
          console.log(`loadSectionData:
						Case 'boosted-announcements-management' matched.
						Calling loadBoostedAnnouncementsData.`);
          // Call the specific function
          await loadBoostedAnnouncementsData();
          // Pagination for boosted announcements
          // is not implemented yet,
          // so we don't set currentCollection or totalItems here.
          // Return early as boosted announcements handles
          // its own pagination if any
          return;

        case "pending-approval-management":
          // Unique key for pagination
          currentCollection = "pending-approvals";
          // Sorting is handled within fetchUsers
          // for the combined pending list
          const {
            users: pendingUsers,
            lastDoc: pendingLastDocIndicatesMore,
            totalCount: pendingTotal,
          } = await fetchUsers({
            fetchOnlyPending: true,
            limitCount: itemsPerPage, // Standard items per page
            // startAfterDoc is not used here due
            // to combined collection fetching complexity
            // for pending
          });
          renderPendingApprovalTable(pendingUsers);
          // For pending approvals, lastVisibleDoc is a boolean
          // indicating if more might exist
          // beyond the current fetch limit
        ...
        case "reported-ads-management": // New case
          ...
          currentCollection = "reported-activities";
          currentPage = 1;
          lastVisibleDoc = null;
          const {
            docs: reportedActivities,
            lastDoc: reportedLastDoc,
            totalCount: reportedTotal,
          } = await fetchReportedActivities({
            limitCount: itemsPerPage,
            // Potentially add sortBy:
            // 'lastReportedAt', sortOrder: 'desc' later
          });
          renderReportedAdsTable(reportedActivities);
          lastVisibleDoc = reportedLastDoc;
          totalItems = reportedTotal;
          // Use a unique identifier for pagination
          updatePagination("reported-activities");
          break;
        ...
        case "settings":
          // Add any additional settings-related logic
          // you want to execute
          break;
      }
      ...
      // Still update pagination (will show 0)
      updatePagination(currentCollection);
    }
  };
  ...
  const updatePagination = (collection) => {
    ...
    prevButton.addEventListener("click", () => {
      if (currentPage > 1) {
        ...
        // assuming fetch handles current page state
        console.warn(
          `Firestore 'previous page' pagination 
		  not fully implemented with startAfter/endBefore. 
		  Reloading current section.`
        );
    ...
    // Disable if on last page or no more docs indication
    nextButton.disabled = currentPage >= totalPages || !lastVisibleDoc;
    nextButton.addEventListener("click", () => {
      if (currentPage < totalPages && lastVisibleDoc) {
        currentPage++;
        // Reload data for the next page
        loadSectionData(getActiveSectionId());
      }
    });
  ...
  if (mainContentArea) {
    mainContentArea.addEventListener("click", async (e) => {
      console.log(
        "Click detected inside mainContentArea:",
        e.target,
      ...
      if (
        targetButton.classList.contains("btn-approve") &&
        id &&
        tableId === "user-table"
      ) {
        e.stopPropagation();
        console.log(`Approve button clicked for user ${id}
					from user-table`);
        // For the main user table, we assume it's a professional
        // if this button is active and it's not already approved.
        // This might need refinement if other user types get a direct
        // approve button here.
        const userToApprove = await getUserData(id);
        if (
          userToApprove &&
          userToApprove.userType === "professional" &&
          !userToApprove.isApproved
        ) {
          showConfirmation(
            "Confirm Approval",
            `Are you sure you want to approve professional 
						${userToApprove.businessName || id}?`,
            () => {
              // Pass current section
              approveUser(id, "professional", "user-management");
            }
          );
        } else {
          showSnackbar(
            `User is not a pending professional 
						or already approved.`,
            "info"
          );
        }
      ...
      if (
        targetButton.classList.contains("btn-approve-pending") &&
        id &&
        tableId === "pending-approval-table"
      ) {
        e.stopPropagation();
        const userTypeToApprove = targetButton.dataset.usertype;
        const userNameToApprove =
          targetButton.closest("tr")?.querySelector("td:first-child div")
            ?.textContent || id;
        ...
        console.log(`Approve button clicked for user ${id} 
					(type: ${userTypeToApprove}) from pending-approval-table`);
        showConfirmation(
          "Confirm Approval",
          `Are you sure you want to approve ${userTypeToApprove} 
					'${userNameToApprove}'?`,
          () => {
      ...
      // (for user-table and pending-approval-table) ---
      if (
        targetButton.classList.contains("view-details-btn") &&
        id &&
        ...
        if (fetchedUser) {
          // Use generic display for all types in pending,
          // and specific for professionals in user management
          // if needed
          if (
            tableId === "pending-approval-table" ||
            (fetchedUser.userType !== "professional" &&
              tableId === "user-table")
          ) {
            ...
            showDetailsModal(
              `Details: ${
                ...
                fetchedUser.arenaName ||
                fetchedUser.email ||
                fetchedUser.id
              }`,
              detailsHtml
            );
            // Specific display for professionals from user-table
          } else if (fetchedUser.userType === "professional") {
            const detailsHtml = populateProfessionalDetails(fetchedUser);
            showDetailsModal(
              `Professional Details: 
							${fetchedUser.businessName || fetchedUser.email}`,
              detailsHtml
            );
          } else {
            ...
            showDetailsModal(
              `Details: 
							${fetchedUser.email || fetchedUser.id}`,
              detailsHtml
            );
          }
        } else {
          ...
          console.error(`Failed to fetch user ${id} 
						for details modal from table ${tableId}.`);
        }
        return;
      }

      // --- Existing View/Edit/Delete Actions ---
      // Ensure these do not conflict. If the 'view-btn'
      // for non-professionals
      // has a different purpose,
      // it will be handled by the existing logic below.

      // View Action (Generic - for non-professionals
      // or if professional view-details-btn is not hit)
      if (targetButton.classList.contains("view-btn") && id) {
        e.stopPropagation();
        console.log(`View button (generic) clicked for ${id}`);
        // This fetches from any collection
      ...
      if (targetButton.classList.contains("edit-btn") && id) {
        ...
        if (tableId === "user-table") {
          ...
          if (user) {
            ...
            document.getElementById("user-modal-type").value =
              user.userType || "individual";
            // Populate other fields based on user type
            openModal(userModal);
          } else {
          ...
          if (facilityRef.exists) {
            ...
            document.getElementById("facility-arena-id").value =
              facility.arenaId || ""; // Preserve arena ID
        ...
        // Add similar logic for editing Activities,
        // Events, Clubs, Teams, Arenas
        else {
          alert(`Edit action for this table (${tableId})
      ...
      // --- View Actions --- New Section
        ...
        showDetailsModal(title, contentHtml);
        ...
        try {
          ...
          if (tableId === "user-table") {
            ...
            formattedContent = `<p>
								Viewing details for this item type 
								is not implemented.
							</p>`;
          }
          ...
          if (data) {
            ...
            // If data fetch failed or type not handled
          ...
          showDetailsModal(
            "Error",
            "<p>Could not load details due to an error.</p>"
          );
        }
      }
      ...
      // --- Delete Actions ---
        ...
        showConfirmation(
          `Delete ${itemType}?`,
          `Are you sure you want to delete this ${itemType} (${id})?
					This cannot be undone.`,
          async () => {
            let success = false;
            // Track if we need to reload facilities
            let arenaIdToReloadFacilities = null;

            switch (itemType) {
              case "user":
                success = await firebaseDeleteUser(id);
                break;
              case "activity":
                success = await firebaseDeleteActivity(id);
                break;
              case "event":
                success = await firebaseDeleteEvent(id);
                break;
              case "club":
                success = false;
                alert("Club deletion not implemented. ");
                break; // Placeholder
              case "team":
                success = false;
                alert("Team deletion not implemented.");
                break; // Placeholder
              case "arena":
                success = false;
                alert("Arena deletion not implemented.");
                break; // Placeholder
              case "facility":
                // Need arenaId to reload facilities
                // after deleting one
                ...
                if (facilityDoc.exists) {
                  arenaIdToReloadFacilities = facilityDoc.data()?.arenaId;
                }
                success = await firebaseDeleteFacility(id);
                break;
              case "conversation":
                success = await firebaseDeleteConversation(id);
                break;
              case "sport":
                success = await firebaseDeleteSport(id);
                break;
              default:
            ...
            if (success) {
              // If deleting a facility,
              // reload just the facility list for that arena
              if (itemType === "facility" && arenaIdToReloadFacilities) {
                const arenaNameSpan = document.getElementById(
                  "selected-arena-name"
                );
                loadFacilitiesForArena(
                  arenaIdToReloadFacilities,
                  arenaNameSpan?.textContent || "Arena"
                );
              } else {
                // Reload the main section table
      ...
      // --- Status Update Actions ---
        ...
        if (tableId === "user-table") {
          ...
          options = {
            [`boost_${!isBoosted}`]: {
              field: "isBoosted",
              value: !isBoosted,
              text: `${!isBoosted ? "Boost" : "Remove Boost"}`,
            },
          };
          ...
          showConfirmation(
            `Confirm Booking ${newStatus}?`,
            `Do you want to mark booking ${id} as ${newStatus}?`,
            async () => {
              const success = await firebaseUpdateBookingStatus(id, newStatus);
              if (success) loadSectionData("booking-management");
        ...
        if (Object.keys(options).length > 0) {
          ...
          console.log(`No status actions available
						for this item/state.`);
        }
      }
      ...
      // --- View Bank Details Action --- New
        ...
        const payoutInfoContainer = document.getElementById(
          "payout-info-container"
        );
        const payoutInfoContent = document.getElementById(
          "payout-info-content"
        );

        if (!entityType) {
          showSnackbar(
            `Cannot determine entity type for payout info.`,
            "error"
          );
          return;
        }
        ...
        if (payoutInfoContainer) {
          ...
          if (payoutInfoContent)
            payoutInfoContent.innerHTML = "<p>Loading payout info...</p>";
          payoutInfoContainer.style.display = "block";

          try {
            const collectionName =
              entityType === "arena" ? "arenas" : "professionals";
            const docSnap = await db.collection(collectionName).doc(id).get();
            let bankDetails = {};
            if (docSnap.exists) {
              const data = docSnap.data() || {};
              // Extract bank details
              // - this might differ between professionals
              // and arenas
              // For now, using the same professional keys
              // as a starting point
              const relevantKeys = [
                "iban",
                "bic",
              ...
              console.warn(`${entityType} document ${id} 
								not found when fetching bank details.`);
            }
            ...
            if (payoutInfoContent)
              payoutInfoContent.innerHTML =
                "<p>Error loading bank details.</p>";
          }
        } else {
      ...
      // --- Mark Paid Action --- New
        ...
        const formattedBalance = currentBalance.toLocaleString("fr-FR", {
          style: "currency",
          currency: "EUR",
        });
        ...
        if (currentBalance <= 0) {
          alert(`Balance is already zero or less. 
						Cannot mark as paid.`);
          return;
        }
        ...
        showConfirmation(
          ...
          `Are you sure you want to mark the balance 
					of ${formattedBalance} as paid? 
					This will set the ${entityType}\'s balance to 0.00 €. `,
          async () => {
            console.log(`Attempting to mark 
							${entityType} ${id} as paid.`);
            const success = await firebaseMarkBalancePaid(id, entityType);
            if (success) {
              alert(`${entityName} marked as paid. 
								Their balance is now zero.`);
              // Reload this specific section
              ...
              alert(`Failed to mark ${entityName} as paid. 
  ...
  if (statusForm) {
    statusForm.addEventListener("submit", async (e) => {
      ...
      // This is the key like 'verify_true' or 'status_Reviewed'
      const selectedOptionKey = document.getElementById("status-select").value;
      const selectElement = document.getElementById("status-select");
      const selectedOption = selectElement.options[selectElement.selectedIndex];
      // Get the actual value ('true', 'false', 'reviewed')
      const selectedValue = selectedOption.dataset.value;
      // Get the field name ('isVerified', 'status')
      const fieldToUpdate = selectedOption.dataset.field;
      ...
      console.log(`Updating ${itemType} ${itemId}: 
				Setting ${fieldToUpdate} to ${updateData[fieldToUpdate]}`);

      try {
        switch (itemType) {
          case "user":
            success = await firebaseUpdateUser(itemId, updateData);
            break;
          case "activity":
            success = await firebaseUpdateActivity(itemId, updateData);
            break;
          case "facility":
            const facilityDoc = await db
              .collection("facilities")
              ...
              .get();
            if (facilityDoc.exists)
              arenaIdForFacilityReload = facilityDoc.data()?.arenaId;
            success = await firebaseUpdateFacility(itemId, updateData);
            break;
          // Assuming approval on user doc
          case "arena":
            success = await firebaseUpdateUser(itemId, updateData);
            break;
        }
        ...
        if (success) {
          ...
          if (itemType === "facility" && arenaIdForFacilityReload) {
            loadFacilitiesForArena(
              arenaIdForFacilityReload,
              document.getElementById("selected-arena-name")?.textContent ||
                "Arena"
            );
          } else {
            loadSectionData(sectionToReload);
  ...
  const showConfirmation = (title, message, onConfirm) => {
    ...
    if (!confirmBtn || !cancelBtn) {
      console.error(`Confirmation modal buttons not found! 
				Check IDs: confirm-action-btn, cancel-action-btn`);
  ...
  const showStatusModal = (title, itemId, itemType, options) => {
    ...
    if (Object.keys(options).length === 0) {
      ...
      for (const [key, optionData] of Object.entries(options)) {
        ...
        // Key identifies the action (e.g., 'verify_true')
        optionElement.value = key;
        // User-friendly text
        optionElement.textContent = optionData.text;
        // Store field name
        optionElement.dataset.field = optionData.field;
        // Store actual value
        optionElement.dataset.value = optionData.value.toString();
  ...
  const loadTeamsForClub = async (clubId, clubName) => {
    ...
    try {
      ...
      const teams = await fetchTeamsFromFirestore(clubId);
      ...
      if (teamTableBody)
        teamTableBody.innerHTML = `
			<tr>
				<td colspan="5">Error loading teams.</td>
			</tr>`;
    }
  ...
  const loadFacilitiesForArena = async (arenaId, arenaName) => {
    ...
    document.getElementById("add-facility-btn").dataset.arenaId = arenaId;
    const facilityTableBody = document
      .getElementById("facility-table")
      ?.querySelector("tbody");
    if (facilityTableBody)
      facilityTableBody.innerHTML =
        '<tr><td colspan="4">Loading facilities...</td></tr>';
  ...
  // Basic debounce might be needed
  ...
  document.getElementById("activity-search")?.addEventListener("input", () => {
    currentPage = 1;
    lastVisibleDoc = null;
    loadSectionData("activity-management");
  });
  document.getElementById("event-search")?.addEventListener("input", () => {
    currentPage = 1;
    lastVisibleDoc = null;
    loadSectionData("event-management");
  });
  document.getElementById("club-search")?.addEventListener("input", () => {
    currentPage = 1;
    lastVisibleDoc = null;
    loadSectionData("club-management");
  });
  document.getElementById("arena-search")?.addEventListener("input", () => {
    currentPage = 1;
    lastVisibleDoc = null;
    loadSectionData("arena-management");
  });
  document.getElementById("booking-search")?.addEventListener("input", () => {
    currentPage = 1;
    lastVisibleDoc = null;
    loadSectionData("booking-management");
  });
  document
  ...
  document.getElementById("chat-search")?.addEventListener("input", () => {
    currentPage = 1;
    lastVisibleDoc = null;
    loadSectionData("chat-management");
  });
  document
    .getElementById("pro-balance-search")
    ?.addEventListener("input", () => {
      currentPage = 1;
      lastVisibleDoc = null;
      loadSectionData("professional-balances");
    });
  // Add listener for new search
  ...
  function loadAdminData(user) {
    ...
    loadSectionData("dashboard-overview");
  }
  ...
  document.getElementById('admin-login-form')?.addEventListener('submit', async (e) => {
    ...
    if (!result.success) {
      ...
      // Dashboard will be shown automatically via auth state listener
    }
  });
  ...
  document.getElementById('logout-button')?.addEventListener('click', async (e) => {
    ...
    if (result.success) {
      // Login screen will be shown automatically via auth state listener
    } else {
      ...
      if (Object.hasOwnProperty.call(data, key)) {
        ...
        if (value && value.toDate) {
          ...
          if (value.length === 0) {
            ...
            // Simple array to comma-separated string
  ...
  const fetchFacilitiesFromFirestore = async (arenaId) => {
    ...
    try {
      ...
      console.log(`Found ${facilities.length} 
				facilities for arena ${arenaId}`);
      return facilities;
    } catch (error) {
  ...
  const firebaseMarkBalancePaid = async (entityId, entityType) => {
    console.log(`Marking ${entityType} ${entityId} 
			as paid (setting balance to 0)`);
    if (!entityId || !entityType) {
      console.error(`Missing entityId or entityType 
				for firebaseMarkBalancePaid`);
      return false;
    }
    ...
    if (entityType === "professional") {
      ...
      console.error(`Unsupported entityType: ${entityType} 
				for firebaseMarkBalancePaid`);
      return false;
    }
    ...
    try {
      ...
      console.log(`Successfully set balance to 0 for 
				${entityType} ${entityId}`);
      return true;
    } catch (error) {
      console.error(
        `Error updating balance for 
				${entityType} ${entityId}:`,
        error
      );
      return false;
  ...
  const approveUser = async (
    userId,
    userType,
    sectionToReload = "user-management"
  ) => {
    console.log(`Attempting to approve ${userType} 
				user: ${userId}, will reload ${sectionToReload}`);
    ...
    try {
      ...
      console.log(`Successfully approved ${userType} user: ${userId} 
					in collection ${collectionPath}`);
      ...
      console.error(
        `Error approving ${userType} user 
					${userId} in ${collectionPath}:`,
        error
      );
      showSnackbar(
        `Error approving ${userType}. 
  ...
  // Removed detailsContainer argument
  const populateProfessionalDetails = (user) => {
    ...
    const formatDateForDetails = (timestamp) => {
      ...
      // Check if it might be a string date
      // from older data or direct input
      const date = new Date(timestamp);
      if (!isNaN(date.valueOf())) {
        // Check if it's a valid date
        return date.toLocaleString();
      }
      // Fallback if not a recognizable date/timestamp
    ...
    detailsHtml += `
			<div class="detail-item">
				<strong>APE Code:</strong> ${user.apeCode || "N/A"}
			</div>`;
    detailsHtml += `
			<div class="detail-item">
    ...
    detailsHtml += `
			<div class="detail-item">
				<strong>Sport Activity:</strong> ${user.sportActivity || "N/A"}
			</div>`;
    detailsHtml += `
			<div class="detail-item">
				<strong>KBIS Number:</strong> ${user.kbisNumber || "N/A"}
			</div>`;
    detailsHtml += `
			<div class="detail-item">
				<strong>Verified:</strong> 
				<span class="status-badge status-${
          user.isVerified === true ? "active" : "inactive"
        }">
    ...
    detailsHtml += `
		<div class="detail-item">
			...
			${formatDateForDetails(user.createdAt)}
		</div>`;
    detailsHtml += `
		<div class="detail-item">
			<strong>Last Updated:</strong> 
			${formatDateForDetails(user.updatedAt)}</div>`;
  ...
  const boostedAnnouncementsList = document.getElementById(
    "boosted-announcements-list"
  );
  // Commented out/Removed
  // const boostedCreatorTypeFilter =
  // document.getElementById('boosted-creator-type-filter');
  const boostedActivitySearch = document.getElementById(
    "boosted-activity-search"
  );
  // Add pagination elements if needed, similar to other sections

  async function fetchCreatorDetails(creatorId, creatorType) {
    // Default for missing basic info
    if (!creatorId || !creatorType) return "Unknown Creator";
    try {
      let docRef;
      let collectionName = "";
      if (creatorType === "professional") {
        ...
        // Assuming general users collection for individuals
        collectionName = "users";
        docRef = db.collection(collectionName).doc(creatorId);
      } else {
        // More specific unknown
        return `Unknown Creator Type (${creatorType})`;
      }
      const doc = await docRef.get();
      if (doc.exists) {
        const data = doc.data();
        // Default to ID if no name field
        return (
          data.businessName ||
          data.clubName ||
          data.name ||
        ...
        // More specific not found
        return `Creator (ID: ${creatorId}, 
					Type: ${creatorType}) Not Found`;
      }
    } catch (error) {
      console.error(
        `Error fetching creator details for 
				${creatorType} ${creatorId}:`,
        error
      );
      // More specific error
      return `Error Fetching Creator (ID: ${creatorId})`;
    }
  }
  ...
  async function renderBoostedAnnouncementCard(activity) {
    ...
    const rawCreatorName = await fetchCreatorDetails(
      activity.creatorId,
      activity.creatorType
    );
    const expiryDate =
      activity.boostExpiryDate && activity.boostExpiryDate.toDate
        ? formatDate(activity.boostExpiryDate.toDate())
        : "N/A";

    let statusText = "Inactive";
    // Default color for inactive
    let statusColor = "var(--dark-secondary-text)";
    if (
      activity.boostExpiryDate &&
      activity.boostExpiryDate.toDate &&
      activity.boostExpiryDate.toDate() > new Date()
    ) {
      statusText = "Active";
    ...
    if (!isProblematicCreatorName) {
      creatorInfoHtml = `
				<p class="creator-info">By: 
					<strong>${rawCreatorName}</strong> 
					(${activity.creatorType || "N/A"})
				</p>`;
    } else {
      // Optionally log that problematic creator name was handled,
      // or display a very generic placeholder if needed
      console.warn(`Hiding problematic creator details for activity 
				${activity.id}: ${rawCreatorName}`);
    }
    ...
    card.innerHTML = `
            ...
            <div class="banner-card-content">
                ...
                ${creatorInfoHtml} 
				{/* This will be empty if creator name is problematic */}
                <p class="activity-type">Type: ${activity.type || "N/A"}</p>
                <p class="boost-expiry">Expires: ${expiryDate}</p>
                <p class="boost-status" style="color: ${statusColor};">
            ...
            <div class="banner-card-actions">
                <button
					class=
						"btn btn-secondary btn-small view-activity-details-btn"
						data-id="${activity.id}"
                ...
                ${
                  statusText === "Active"
					...
					<button
						class="
							btn btn-danger btn-small deactivate-boost-btn"
							data-id="${activity.id}"
						title="Deactivate Boost">
							Deactivate
					</button>`
                    : ""
                }
    ...
    // Event listeners for action buttons (ensure these functions
    // exist or are placeholders)
    const viewDetailsBtn = card.querySelector(".view-activity-details-btn");
    if (viewDetailsBtn) {
      viewDetailsBtn.addEventListener("click", () => {
        // Placeholder or actual implementation for
    ...
    if (statusText === "Active") {
      ...
      if (deactivateBtn) {
        deactivateBtn.addEventListener("click", () => {
          // Placeholder or actual implementation for
          // deactivateBoost(activity.id)
          console.log("Deactivate boost clicked for activity ID:", activity.id);
          showConfirmation(
            "Deactivate Boost?",
            `Are you sure you want to deactivate the boost for 
						${activity.name || activity.id}"?`,
            async () => {
              try {
                await db.collection("activities").doc(activity.id).update({
                  isBoosted: false,
                  // Or set to null/past date
                  // boostExpiryDate:
                  // firebase.firestore
                  // 	.FieldValue.delete()
                });
                console.log(`Boost deactivated for activity 
									${activity.id}`);
  ...
  async function loadBoostedAnnouncementsData() {
    if (!boostedAnnouncementsList) {
      console.log(`Boosted announcements list element not found.
				Exiting.`);
      return;
    }
    console.log("loadBoostedAnnouncementsData: Setting loading message.");
    boostedAnnouncementsList.innerHTML =
      '<p class="loading-text">Loading boosted announcements...</p>';

    try {
      console.log(`loadBoostedAnnouncementsData: 
				Attempting to fetch boosted activities from Firestore.`);
      const activitiesSnapshot = await db
        .collection("activities")
        .where("isBoosted", "==", true)
        .orderBy("boostExpiryDate", "desc")
        .get();
      console.log(`loadBoostedAnnouncementsData: Fetched 
				${activitiesSnapshot.docs.length} 
				boosted activities from Firestore.`);
      ...
      const searchTerm = boostedActivitySearch
        ? boostedActivitySearch.value.toLowerCase()
        : "";
      console.log(`loadBoostedAnnouncementsData: Search term is '
				${searchTerm}'.`);
      ...
      if (searchTerm) {
        ...
        filteredActivities = filteredActivities.filter(
          (act) =>
            ...
            (act.creatorId &&
              act.creatorId.toLowerCase().includes(searchTerm)) ||
            (act.creatorName &&
              act.creatorName.toLowerCase().includes(searchTerm))
        );
        console.log(`loadBoostedAnnouncementsData: Found 
					${filteredActivities.length} 
					activities after search filter.`);
      }

      if (filteredActivities.length === 0) {
        console.log(`loadBoostedAnnouncementsData: 
					No filtered activities. 
					Setting 'no announcements' message.`);
        boostedAnnouncementsList.innerHTML = `
					<p>
						No boosted announcements found matching your criteria.
					</p>`;
        return;
      }

      console.log(`loadBoostedAnnouncementsData: 
				About to render ${filteredActivities.length} activity cards.`);
      // Clear loading text before loop
      boostedAnnouncementsList.innerHTML = "";

      for (let i = 0; i < filteredActivities.length; i++) {
        const activity = filteredActivities[i];
        console.log(`loadBoostedAnnouncementsData: 
					Rendering card ${i + 1} of ${filteredActivities.length} 
					for activity ID ${activity.id}`);
        try {
          const card = await renderBoostedAnnouncementCard(activity);
          boostedAnnouncementsList.appendChild(card);
          console.log(`loadBoostedAnnouncementsData: 
						Successfully appended card for activity ID 
						${activity.id}`);
        } catch (renderError) {
          console.error(
            `Error rendering card for activity 
						${activity.id} (name: ${activity.name || "N/A"}):`,
            renderError
          );
          const errorCardMsg = document.createElement("p");
          errorCardMsg.textContent = `Error loading card for: 
					${activity.name || activity.id}. Check console.`;
      ...
      console.error(
        `loadBoostedAnnouncementsData: CATCH BLOCK HIT 
				- Main error loading boosted announcements: `,
        error
      );
      boostedAnnouncementsList.innerHTML = `
			<p class="error-text">
  ...
  //     boostedCreatorTypeFilter.addEventListener('change',
  // 		loadBoostedAnnouncementsData);
  // }
  ...
  // This assumes navLinks and contentSections
  // are already defined and used in a loop
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    navLinks.forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        // Assuming loadUsersData exists
        if (sectionId === "user-management") loadUsersData();
        // Assuming loadActivitiesData exists
        else if (sectionId === "activity-management") loadActivitiesData();
        // Assuming loadEventsData exists
        else if (sectionId === "event-management") loadEventsData();
        // Assuming loadClubsData exists
        else if (sectionId === "club-management") loadClubsData();
        // Assuming loadArenasData exists
        else if (sectionId === "arena-management") loadArenasData();
        // Assuming loadBookingsData exists
        else if (sectionId === "booking-management") loadBookingsData();
        // Assuming loadProfessionalBalancesData exists
        else if (sectionId === "professional-balances")
          loadProfessionalBalancesData();
        // Assuming loadChatsData exists
        else if (sectionId === "chat-management") loadChatsData();
        else if (sectionId === "boosted-announcements-management")
          loadBoostedAnnouncementsData();
        // ... other sections ...
        // Assuming loadDashboardOverviewData exists
        else if (sectionId === "dashboard-overview")
          loadDashboardOverviewData();
      });
    });

    // Trigger click on the default active link (e.g., dashboard)
    // to load its data if not already handled
    const defaultActiveLink = document.querySelector(
      ".sidebar-nav a.nav-link.active"
    );
    if (defaultActiveLink) {
      // Simplified initial load trigger
      const sectionId = defaultActiveLink.getAttribute("data-section");
      // Simplified logic: just check the sectionId
      // for relevant load functions
      if (
        sectionId === "dashboard-overview" &&
        typeof loadDashboardOverviewData === "function"
      )
        loadDashboardOverviewData();
      else if (
        sectionId === "user-management" &&
        typeof loadUsersData === "function"
      )
        loadUsersData();
      else if (
        sectionId === "activity-management" &&
        typeof loadActivitiesData === "function"
      )
        loadActivitiesData();
      else if (
        sectionId === "event-management" &&
        typeof loadEventsData === "function"
      )
        loadEventsData();
      else if (
        sectionId === "club-management" &&
        typeof loadClubsData === "function"
      )
        loadClubsData();
      else if (
        sectionId === "arena-management" &&
        typeof loadArenasData === "function"
      )
        loadArenasData();
      else if (
        sectionId === "booking-management" &&
        typeof loadBookingsData === "function"
      )
        loadBookingsData();
      else if (
        sectionId === "professional-balances" &&
        typeof loadProfessionalBalancesData === "function"
      )
        loadProfessionalBalancesData();
      else if (
        sectionId === "chat-management" &&
        typeof loadChatsData === "function"
      )
        loadChatsData();
      else if (sectionId === "boosted-announcements-management")
        loadBoostedAnnouncementsData();
      ...
      if (typeof loadDashboardOverviewData === "function")
        loadDashboardOverviewData();
    }
    // ... other initializations ...
  });
  ...
  // (assuming it's done elsewhere, e.g., in firebase-init.js or at the top)
  // if (typeof db === 'undefined') { const db = firebase.firestore(); }

  // Add placeholder functions for actions (to be implemented later)
  // function viewActivityDetails(activityId)
  // { console.log('View details for:', activityId);
  // /* Show modal or navigate */ }
  // function deactivateBoost(activityId)
  // { console.log('Deactivate boost for:', activityId);
  // /* Update Firestore */ }
  ...
  if (mainContentArea) {
    mainContentArea.addEventListener("click", async (event) => {
      ...
      // Used when adding/editing facility from arena context
      const facilityArenaId = target.dataset.arenaId;
        ...
        if (activityId) {
          showConfirmation(
            ...
            async () => {
              ...
              // Assumes firebaseDeleteActivity
              // handles UI update or snackbar
              await firebaseDeleteActivity(activityId);
    ...
    deleteAdBtnInModal.addEventListener("click", () => {
      ...
      if (adId) {
        showConfirmation(
          "Delete Reported Ad",
          `Are you sure you want to permanently delete this ad 
		  from the modal? This action cannot be undone.`,
          async () => {
  ...
  const renderSportsTable = (sports) => {
    ...
    sports.forEach((sport) => {
      ...
      row.innerHTML = `
                ...
                <td>
					${sport.createdAt ? formatDate(sport.createdAt) : "N/A"}
				</td>
                <td class="actions">
                    <button
						class="btn btn-small btn-danger delete-btn"
						data-id="${sport.id}"
						data-name="${sport.name || "this sport"}"
						title="Delete Sport">
						<i class="fas fa-trash"></i>
					</button>
                </td>
  ...
  const firebaseAddSport = async (sportName) => {
    ...
    try {
      ...
      const existingSportQuery = await db
        .collection("sports")
      ...
      await db.collection("sports").add({
        ...
        nameLower: sportName.trim().toLowerCase(),
  ...
  if (addSportBtn && newSportNameInput) {
    addSportBtn.addEventListener("click", async () => {
      // Added for debugging
      console.log(
        `[DEBUG] Add Sport Button Clicked. 
				Value of newSportNameInput.value:`,
    ...
    newSportNameInput.addEventListener("keypress", async (e) => {
      if (e.key === "Enter") {
        console.log(
          `[DEBUG] Enter Key Pressed in Sport Input. 
					Value of newSportNameInput.value:`,
          newSportNameInput.value
        ); // Added for debugging
        const sportName = newSportNameInput.value;
        ...
        if (success) {
          ...
          if (getActiveSectionId() === "sports-management") {
            // Reload list
            await loadSectionData("sports-management");
          }
        }
      }
    });
